...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  /* Shared admin components used by all pages */
  .stats-row{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:14px;margin-bottom:22px}
  .stat{background:#fff;border:1px solid #eef0f5;border-radius:14px;padding:16px;display:flex;align-items:center;gap:12px;min-width:0;box-shadow:0 2px 10px rgba(16,24,40,.04)}
  .stat-ico{width:44px;height:44px;min-width:44px;border-radius:12px;display:grid;place-items:center;font-size:20px}
  .stat-info{min-width:0}.stat-info h3{font-size:22px;line-height:1.1;color:#111827}.stat-info p{font-size:12px;color:#6b7280;margin-top:3px}.stat-info small{font-size:10px;color:#9ca3af;display:block;margin-top:3px;white-space:nowrap}
  .card{background:#fff;border:1px solid #eef0f5;border-radius:14px;box-shadow:0 2px 10px rgba(16,24,40,.04);overflow:hidden;margin-bottom:20px}.card-body{padding:22px}.card-header{padding:16px 20px;border-bottom:1px solid #eef0f5;display:flex;align-items:center;justify-content:space-between;gap:12px}.card-header h2{font-size:16px;color:#111827}.tbl-scroll{width:100%;overflow-x:auto}
  .btn-sm{padding:7px 11px;font-size:12px}.btn-secondary{background:#f3f4f6;color:#374151}.btn-success{background:#dcfce7;color:#166534}.btn-purple{background:#f3e8ff;color:#6b21a8}.btn-gray{background:#f3f4f6;color:#4b5563}
  .bg-green{background:#dcfce7;color:#166534}.bg-red{background:#fee2e2;color:#991b1b}.bg-blue{background:#dbeafe;color:#1e40af}.bg-orange{background:#ffedd5;color:#9a3412}.bg-purple{background:#f3e8ff;color:#6b21a8}.bg-gray{background:#f3f4f6;color:#4b5563}
  .empty{text-align:center;padding:45px 20px;color:#9ca3af}.empty .e-ico{font-size:36px;margin-bottom:10px}.empty p{font-size:14px}
  .pg-btn{padding:6px 10px;border:1px solid #e5e7eb;border-radius:7px;background:#fff;color:#374151;text-decoration:none;font-size:12px;display:inline-flex;align-items:center;justify-content:center;min-width:32px}.pg-btn.active{background:#e91e63;color:#fff;border-color:#e91e63}.pg-btn.disabled{opacity:.45;pointer-events:none}.pg-btn:hover:not(.active):not(.disabled){background:#fce4ec;color:#c2185b}
  .sidebar-toggle{display:none;border:0;background:#fce4ec;color:#c2185b;border-radius:9px;width:38px;height:38px;font-size:20px;cursor:pointer}.sidebar-overlay{display:none}
  @media(max-width:1200px){.stats-row{grid-template-columns:repeat(3,minmax(0,1fr))}}
  @media(max-width:900px){.sidebar{transform:translateX(-100%);transition:transform .25s ease;z-index:1000;box-shadow:8px 0 30px rgba(0,0,0,.15)}.sidebar.open{transform:translateX(0)}.sidebar-overlay{position:fixed;inset:0;background:rgba(15,23,42,.45);z-index:999}.sidebar-overlay.show{display:block}.main{margin-left:0}.topbar{padding:12px 16px}.sidebar-toggle{display:inline-grid;place-items:center}.topbar-left{display:flex;align-items:center;gap:10px}.content{padding:18px}}
  @media(max-width:600px){.stats-row{grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.stat{padding:12px}.stat-ico{width:38px;height:38px;min-width:38px}.stat-info h3{font-size:19px}.dash-hero{padding:20px}.quick-btns{gap:7px}.qb{padding:8px 10px;font-size:12px}.topbar h1{font-size:18px}}
</style>
</head>
<body>
...
<div class="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <a href="advertisements.php" class="nav-item <?= ($pageTitle ?? '') === 'Advertisements' ? 'active' : '' ?>">
      <span class="icon">📢</span> Advertisements
    </a>
    <a href="app_content.php" class="nav-item <?= ($pageTitle ?? '') === 'App Content' ? 'active' : '' ?>">
    ...
    <a href="app_settings.php" class="nav-item <?= ($pageTitle ?? '') === 'App Settings' ? 'active' : '' ?>">
      <span class="icon">📱</span> App Settings
    </a>
...
<div class="main">
  <div class="topbar">
    <div class="topbar-left">
      <button type="button" class="sidebar-toggle" onclick="toggleSidebar()" aria-label="Open menu">☰</button>
