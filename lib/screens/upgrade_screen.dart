...
class _UpgradeScreenState extends State<UpgradeScreen> {
  ...
  Future<void> _startPayment(Map<String, dynamic> plan) async {
    ...
    // Step 1: Create order on backend → get Cashfree Payment Session ID
    final planId = int.tryParse(plan['id'].toString()) ?? 0;
    final orderRes = await ApiService.createPaymentOrder(planId: planId);

    if (!mounted) return;

    final orderOk = orderRes['status'] == true || orderRes['success'] == true;
    if (!orderOk) {
      setState(() => _isPurchasing = false);
      final errMsg = orderRes['message']?.toString() ?? 'Could not create payment order';
      _showSnack('Order Error: $errMsg', isError: true);
      return;
    }
    ...
    final cfOrderId        = data['cf_order_id']?.toString() ?? '';
    final environment      = data['environment']?.toString() ?? _gatewayEnv;
    final rawAmount        = data['amount'];
    final amount      = (rawAmount is num)
        ? rawAmount.toDouble()
        : double.tryParse(rawAmount?.toString() ?? '0') ?? 0.0;

    if (paymentSessionId.isEmpty || cfOrderId.isEmpty) {
      setState(() => _isPurchasing = false);
      _showSnack('Session ID missing. Response: ${orderRes.toString().substring(0, orderRes.toString().length > 200 ? 200 : orderRes.toString().length)}', isError: true);
      return;
    }

    setState(() => _isPurchasing = false);

    // Step 2: Open Cashfree's official Flutter checkout.
